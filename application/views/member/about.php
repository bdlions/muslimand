
<!--BODY-->
<div class="container-fluid" style="background-color: #E9EAED">
    <div class="row">
        <div class="col-md-1">
        </div>

        <div class=" col-md-8 ">
            <?php $this->load->view("member/pagelets/profile_cover"); ?>

            <div class="row form-group"></div>
            <div class="pagelet">
                <div class="row">
                    <div class="col-md-1">
                        <div class="row form-group"></div>
                        <img src="<?php echo base_url(); ?>resources/images/about.png"  width="28" height="28">   
                    </div>
                    <div class="col-md-11">
                        <h3>About</h3>  
                    </div>
                </div>
            </div>
            <div class="pagelet">
                <div class="row">
                    <div class="col-md-3" style="border-right: 1px solid lightgray;">
                        <div class="pagelet">
                            <div class="row">
                                <div class="col-md-12" >
                                    <a style=" color: black;" href="#">Overview</a>
                                </div>
                            </div>
                            <div class="pagelet_divider"></div>
                            <div class="row">
                                <div class="col-md-12">
                                    <a style=" color: black;" href="#"> Works and Education</a>
                                </div>
                            </div>
                            <div class="pagelet_divider"></div>
                            <div class="row">
                                <div class="col-md-12">
                                    <a style=" color: black;" href="#"> Places you've lived</a>
                                </div>
                            </div>
                            <div class="pagelet_divider"></div>
                            <div class="row">
                                <div class="col-md-12">
                                    <a style=" color: black;" href="#"> Contacts and Basic Info</a>
                                </div>
                            </div>
                            <div class="pagelet_divider"></div>
                            <div class="row">
                                <div class="col-md-12">
                                    <a style=" color: black;" href="#"> Family and Relationships</a>
                                </div>
                            </div>
                            <div class="pagelet_divider"></div>
                            <div class="row">
                                <div class="col-md-12">
                                    <a style=" color: black;" href="#"> Details About You</a>
                                </div>
                            </div>
                            <div class="pagelet_divider"></div>
                            <div class="row">
                                <div class="col-md-12">
                                    <a style=" color: black;" href="#"> Life Events</a>
                                </div>
                            </div>
                        </div>
                    </div> 
                    <div class="col-md-9">
                        <div class="pagelet">
                            <div class="row">
                                <div class="row form-group"></div>
                                <div class="col-md-7">
                                    <div class="row">

                                        <div class="col-md-2">
                                            <img src="<?php echo base_url(); ?>resources/images/car.jpg"  width="40" height="40"> 
                                        </div>
                                        <div class="col-md-10">
                                            <h6>Works at <a style="color: blue;" href="#">Sampan-IT </a></h6>
                                        </div>
                                    </div>
                                    <div class="pagelet_divider"></div>
                                    <div class="row">
                                        <div class="col-md-2">
                                            <img src="<?php echo base_url(); ?>resources/images/Food.jpg"  width="40" height="40"> 
                                        </div>
                                        <div class="col-md-10">
                                            <h6>Studied at <a style="color: blue;" href="#">ABC University </a></h6>
                                        </div>
                                    </div>
                                    <div class="pagelet_divider"></div>
                                    <div class="row">
                                        <div class="col-md-2">
                                            <img src="<?php echo base_url(); ?>resources/images/car_1.jpg"  width="40" height="40"> 
                                        </div>
                                        <div class="col-md-10">
                                            <h6>Lives in <a style="color: blue;" href="#">Dhaka, Bangladesh </a></h6>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-5">
                                    <div class="row form-group"></div>
                                    <div class="row">
                                        <div class="col-md-2">
                                            <img src="<?php echo base_url(); ?>resources/images/mobile_icon.png"  width="20" height="20"> 
                                        </div>
                                        <div class="col-md-10">
                                            <a style="color: blue;" href="#">01XXX XXX XXX </a>
                                        </div>
                                    </div>
                                    <div class="row form-group"></div>
                                    <div class="row">
                                        <div class="col-md-2">
                                            <img src="<?php echo base_url(); ?>resources/images/mail.png"  width="20" height="20"> 
                                        </div>
                                        <div class="col-md-10">
                                            <h6>mail@muslimand.com </h6>
                                        </div>
                                    </div>
                                    <div class="row form-group"></div>
                                    <div class="row">
                                        <div class="col-md-2">
                                            <img src="<?php echo base_url(); ?>resources/images/address.png"  width="20" height="20"> 
                                        </div>
                                        <div class="col-md-10">
                                            <h6>168, Shanti Niketon </h6>
                                        </div>
                                    </div>
                                    <div class="row form-group"></div>
                                    <div class="row">
                                        <div class="col-md-2">
                                            <img src="<?php echo base_url(); ?>resources/images/massenger.png"  width="20" height="20"> 
                                        </div>
                                        <div class="col-md-10">
                                            <a style="color: blue;" href="#">sampan-it </a>
                                        </div>
                                    </div>
                                    <div class="row form-group"></div>
                                    <div class="row">
                                        <div class="col-md-2">
                                            <img src="<?php echo base_url(); ?>resources/images/website.png"  width="20" height="20"> 
                                        </div>
                                        <div class="col-md-10">
                                            <a style="color: blue;" href="#">http://www.muslimand.com/ </a>
                                        </div>
                                    </div>
                                    <div class="row form-group"></div>
                                    <div class="row">
                                        <div class="col-md-2">
                                            <img src="<?php echo base_url(); ?>resources/images/birth.png"  width="20" height="20"> 
                                        </div>
                                        <div class="col-md-10">
                                            <h6>January 1,1990 </h6>
                                        </div>
                                    </div>
                                </div>

                            </div> 
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <!--Left COLUMN (Update status & chat box)-->
        <div class="col-md-offset-1 col-md-2" style="border-left: 1px solid lightgray">

            <?php $this->load->view("member/sections/right_column_ticker_friends"); ?>
        </div>
        <div class="row form-group"></div>
    </div>
</div>
</div>

<!--FOOTER-->
<div class="container-fluid">
    <div class="row">
        <div class="col-md-offset-1 col-md-10">
            <div style="padding: 20px 0px 0px 0px; color: #703684">
                About | Contact | Terms
            </div>
        </div>
    </div>
</div>


